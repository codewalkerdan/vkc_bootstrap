/**
 * @file textured_cube_sdl.c
 * @brief Complete SDL3 sample application rendering a rotating textured 3D cube.
 ...
 * - SDL3 Window Surface integration
...
static bool recreate_swapchain(AppContext* app) {
    ...
    while ((width == 0 || height == 0) && !app->should_quit) {
        process_events(app);
        SDL_GetWindowSizeInPixels(app->window, &width, &height);
        SDL_Delay(16);
    }
...
int main(void) {
    ...
    printf("Initializing SDL3 video...\n");
    if (!SDL_Init(SDL_INIT_VIDEO)) {
        fprintf(stderr, "Failed to initialize SDL3 video: %s\n", SDL_GetError());
        return EXIT_FAILURE;
    }

    app.window = SDL_CreateWindow(
        "vkc-bootstrap: 3D Textured Cube",
    ...
    if (!sdl_extensions) {
        fprintf(stderr, "Failed to get SDL3 Vulkan instance extensions: %s\n", SDL_GetError());
        cleanup_app(&app);
        return EXIT_FAILURE;
    }
    ...
    inst_info.required_extensions = sdl_extensions;
    inst_info.required_extension_count = sdl_extension_count;
     ...
     * ---------------------------------------------------------------------- */
    if (!SDL_Vulkan_CreateSurface(app.window, app.instance.instance, NULL, &app.surface)) {
        fprintf(stderr, "Failed to create SDL3 Vulkan surface: %s\n", SDL_GetError());
        cleanup_app(&app);
        return EXIT_FAILURE;
    }
